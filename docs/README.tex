...
\item v1.03 now includes the option of include Nucleon-Nucleon
  ...
  \code{0}.
\item STIR turbulence can now be combined with rotation. The turbulent
  diffusion operator includes the conserved angular momentum variable
  alongside the existing diffusion of specific internal energy and
  composition, and the STIR source model includes rotational-shear
  production of turbulent kinetic energy based on $r\,d\Omega/dr$.
  The output quantity {\emph{omega2\_BV.xg}} includes the rotational
  Solberg-H\o iland correction to the convective growth rate.
